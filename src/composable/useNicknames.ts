...
export function useNicknames() {
  ...
  const getNicknameOwner = async (name: string): Promise<string> => {
    ...
    const { value } = decodeResult(root, 'common.address', result); // TODO change if decodeResult contains normalizing
    return utils.encodeBase58(utils.decodeBase64(value));
  };
